    ...
    if (userMail != null) {
      try {
        ...
        for (var doc in queryMessageID.docs) {
          final data = doc.data();
          if (data['receiver'] == userMail && data['unread'] == true) {
            ...
            // Tạo đối tượng Message từ Firestore
            final loadedMail = Message(
              ...
              text: data['text'] ?? 'No Content',
              time: DateFormat('hh:mm a').format(DateTime.parse(data['time'])),
              unread: data['unread'] ?? true, // Giữ nguyên trạng thái 'unread'
              isStarred: data['isStarred'] ?? false,
              replies: replyList,
