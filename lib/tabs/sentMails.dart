...
class _SentState extends State<Sent> {
  ...
  Future<void> loadMailsFromFirebase() async {
    if (userMail != null) {
      try {
        // Lấy tất cả các document trong collection 'mails' của người dùng
        final queryMessageID = await FirebaseFirestore.instance
            ...
            .collection(
                'mails') // Truy vấn tới collection 'mails' của người dùng
            .orderBy('time', descending: true) // Sắp xếp theo thời gian nhận
        ...
        for (var doc in queryMessageID.docs) {
          ...
          if (data['sender'] == userMail) {
            print(userMail);
            final colorValue = data['color'].toString() ??
                '0xFF000000'; // Nếu không có giá trị, sử dụng mặc định
            ...
            final loadedMail = Message(
                sender: myModels.AppUser(
                  id: mails.length,
                  name: data['sender'] ?? 'Unknown', // Hiển thị tên người gửi
                  imageUrl: Color(int.parse(
                      colorValue)), // Nếu có màu, hiển thị ảnh đại diện người gửi
                ),
                reciever:
                    data['receiver'] ?? 'No Reciever', // Hiển thị người nhận
                subject: data['subject'] ?? 'No Subject', // Hiển thị tiêu đề
                text: data['text'] ?? 'No Content', // Hiển thị nội dung email
                time: DateFormat('hh:mm a')
                    .format(DateTime.parse(data['time'])), // Hiển thị thời gian
                unread: data['unread'] ?? true, // Kiểm tra trạng thái đã đọc
                isStarred:
                    data['isStarred'] ?? false, // Kiểm tra trạng thái đánh dấu
                replies: replyList,
                threadID: doc.id, // Lưu ID của mail
                attachments: attachments);
            // Cập nhật danh sách email trong UI
            setState(() {
          ...
          // Kiểm tra và xử lý color nếu là String
        }
      } catch (e) {
                  ...
                  StreamBuilder<DocumentSnapshot>(
                    ...
                    builder: (context, snapshot) {
                      ...
                      final data = snapshot.data!.data()
                          as Map<String, dynamic>; // Ép kiểu
                      final avatarUrl = data['avatarUrl'] ?? 'assets/user.png';

                      return PopupMenuButton<String>(
                        icon: CircleAvatar(
                          backgroundImage: avatarUrl.startsWith('http')
                              ? NetworkImage(avatarUrl) // Hiển thị từ URL
                          ...
                          onTap: () => Navigator.push(
                            ...
                            MaterialPageRoute(
                              builder: (_) => Gmail(
                                ...
                                attachments: mails[index - 1].attachments,
                              ),
                            ),
                          ),
                        ),
